...
line 5-11: Bounds check for the SIMD\_float32x4\_load operation. \ttt{ecx}
...
This gives us a basis for comparing SIMD/Scalar speedups as well as absolute 
performance differences between native C++ performance and JavaScript execution 
performance.  For the JavaScript execution we've used our V8 and SpiderMonkey
SIMD enabled prototypes.
...
These are the combinations that we'll present the SIMD vs. Scalar results for:
...
\subsection{SIMD vs. Scalar relative speedups}

Figure~\ref{fig:simd-relative-speedups} shows the relative SIMD vs. Scalar speedups
of the five combinations.

\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{figures/speedup-01.png}
\end{center}
\caption{SIMD vs.Scalar relative speedups}
\label{fig:simd-relative-speedups}
\end{figure}

\subsection{Scalar C++ vs. JavaScript}

Figure~\ref{fig:cpp-js-scalar} shows the relative scalar speedup for each 
of the kernels.  Less than 1 means that the kernel ran that much slower.

\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{figures/scalar-speedup.png}
\end{center}
\caption{SIMD vs.Scalar relative speedups}
\label{fig:cpp-js-scalar}
\end{figure}

\subsection{C++/SIMD vs. JavaScript/SIMD}
