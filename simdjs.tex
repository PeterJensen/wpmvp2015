...
the porting to the Web platform of compute-intensive native C and C++ applications
with near-native performance. The contributions of our work include the changes to
Emscripten to automatically port native C/C++ at near-native speeds when using 
SIMD intrinsics or gcc style vector code. Our contributions are publicly available, and
our evaluation demonstrates that this path for porting native applications with vector code
 can result in equivalent speedups to scalar code in their respective environments.
Future work should consider a study of native C/C++ applications to find vector code
that might not have equivalents in the evolving SIMD.JS specification in order to
contribute to it.
 
C/C++ applications 

 , a new JavaScript standard, that provides low
level data types and operations that nicely map to existing SIMD hardware.  
\acks
